...
public class AllySelectionUI : MonoBehaviour
{
    public static AllySelectionUI Instance;

    public GameObject menuPanel;
    private List<TextMeshProUGUI> options;
    private int currentIndex = 0;
    private Action<int> onOptionSelected;
    public List<TextMeshProUGUI> aliadoTexts;
    ...
    void UpdateVisual()
    {
        ...
        PointerController.Instance.ApontarPara(options[currentIndex].GetComponent<RectTransform>(), new Vector2(-190f, -5f));
    }
    ...
    public void InicializarAliados(List<Character> aliados)
    {
        for (int i = 0; i < aliadoTexts.Count; i++)
        {
            if (i < aliados.Count)
            {
                var a = aliados[i];
                aliadoTexts[i].text = $"{a.characterName}  {a.currentHP}/{a.maxHP} HP  {a.currentMP} MP";
            }
            else
            {
                aliadoTexts[i].text = "";
